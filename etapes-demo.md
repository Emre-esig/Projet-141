...
Le fichier [`public/favicon.png`](public/assets/favicon.png) contient les silhouettes de Rick & Morty (icône officielle de l'API).
...
> On réutilise le fichier [`public/favicon.png`](public/assets/favicon.png) (les silhouettes Rick & Morty) comme logo dans le header. On utilise un `<img>` natif plutôt que `<v-img>` pour éviter les problèmes de layout dans la barre de navigation. Les fichiers dans `public/` sont accessibles directement via `/` dans les URLs.
